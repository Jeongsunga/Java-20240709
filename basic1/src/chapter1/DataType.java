...
public class DataType {
	...
	public static void main(String[] args) {
		...
		char char3 = 100;
		short short2 = (short)char3; // short 중 1비트는 부호 비트지만, char는 모든 비트를 사용
		int int2 = char3;
		...
		// 배열의 길이를 확인하고자 할 땐 .length를 사용한다.
		...
		// compareTo(문자열) : 두 문자열을 사전순으로 비교(대소분자 구분)
		// compareToIgnoreCase(문자열) : 두 문자열을 사전순으로 비교(대소분자 구분 X)
		System.out.println(statement.equals("  My name is loki  "));
		System.out.println(statement.compareTo("  Ay name is loki  "));
		// 비교 문자열이 알파벳순 앞이면 양수, 뒤면 음수, 동일하면 0
		...
		System.out.println(statement.indexOf("사자"));	// 존재하지 않은 값을 찾는다면 -1 반환
		System.out.println(statement.lastIndexOf("기린"));
		System.out.println(statement.lastIndexOf("사자"));
		
		
		// 6. 부분 문자열
		// substring(시작인덱스), substring(시작인덱스, 종료인덱스) : 문자열을 원하는 부분만큼 반환
