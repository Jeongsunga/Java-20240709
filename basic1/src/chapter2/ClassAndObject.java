...
class ExampleClass1{
	...
	int medthod1 (int arg1) {
		// 기능 구현
		// 반환 타입이 void가 아니면 반드시 return으로 결과 값을 반환해야 함
		return arg1 * arg1;
	}
	
	// 클래스 메서드 : 인스턴스 생성없이 클래스로만 호출 가능한 메서드
	// 반환타입 앞에 static 키워드를 붙여서 사용
	// 인스턴스 필드를 사용하지 않는 메서드에 사용됨
...
class TriangleMath{
	...
	// PI, getCircleArea(double)은 단일 책임 원칙(SRP) 위배
...
public class ClassAndObject {
	...
	public static void main(String[] args) {
		...
		// 클래스명 참조변수명 = new 클래스명();
		// instance1이라는 변수에 new ExampleClass1()이라는 인스턴스를 넣어주는 것!
		...
		// 인스턴스.메서드명(매개변수, ...);
		int methodResult = instance1.medthod1(5);
		...
		iphone1.modelName = "Iphone 15 Pro";
