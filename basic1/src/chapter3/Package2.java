...
public class Package2 {
	...
	public static void main(String[] args) {
		...
		// Collection Framework에서 데이터 타입을 지정할 땐 제너릭을 사용하여 지정, 참조형 데이터 타입(Wrapper 클래스)만 지정 가능
		List<Integer> list;
		...
		stringList.remove("apple"); // 없는 값을 삭제하려고 하면, 아무 일 없이 종료
		System.out.println(stringList);
		...
		// contains() : Set에 특정 요소가 존재하는지 확인(논리 형태로 반환)
		...
		// 만약 동일한 키가 존재하면 원래 값을 덮어씌움(수정)
		map.put("Apple", "사과");
		map.put("Banana", "바나나");
		map.put("Banana", "버네이너");
		System.out.println(map);
		
		// get() : Map에서 특정 키를 가지는 요소를 반환 -> 존재하지 않을 때 null 반환
		System.out.println(map.get("apple"));
		
		// containKey() : Map에 특정 키가 존재하는지 확인
